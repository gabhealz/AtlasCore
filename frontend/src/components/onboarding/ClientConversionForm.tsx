...
export function ClientConversionForm({ onboardingId }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const axiosError = err as AxiosError<{ detail?: { message?: string } }>;
  ...
  return (
    <div className="bg-card p-8 rounded-xl shadow-sm border border-brand/10 max-w-2xl mx-auto my-12">
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl font-bold text-ink">Onboarding Finalizado! 🎉</h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="col-span-2">
            <label htmlFor="email" className="block text-sm font-medium text-ink">Email de Contato</label>
            <input
              type="email"
              name="email"
              id="email"
          ...
          <div>
            <label htmlFor="phone" className="block text-sm font-medium text-ink">Celular / WhatsApp</label>
            <input
              type="text"
              name="phone"
              id="phone"
          ...
          <div>
            <label htmlFor="monthly_fee" className="block text-sm font-medium text-ink">Mensalidade (R$)</label>
            <div className="mt-1 relative rounded-md shadow-sm">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <span className="text-muted sm:text-sm">R$</span>
              </div>
              <input
                type="number"
                step="0.01"
                ...
                onChange={handleChange}
                className="pl-9 block w-full rounded-md border-line shadow-sm focus:border-brand focus:ring-brand sm:text-sm p-2.5 border"
                placeholder="2500.00"
              />
            </div>
          </div>
          ...
          <div>
            <label htmlFor="city" className="block text-sm font-medium text-ink">Cidade</label>
            <input
              type="text"
              name="city"
              id="city"
          ...
          <div>
            <label htmlFor="state" className="block text-sm font-medium text-ink">Estado (UF)</label>
            <input
              type="text"
              name="state"
              id="state"
              ...
              onChange={handleChange}
              className="mt-1 block w-full rounded-md border-line shadow-sm focus:border-brand focus:ring-brand sm:text-sm p-2.5 border uppercase"
              placeholder="SP"
            />
          </div>
        </div>
